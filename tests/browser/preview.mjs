...
for (const [name, engine] of [['chromium', chromium], ['webkit', webkit]]) {
  ...
  try {
    ...
    const positions=[await searchPosition()];await search.fill('Amira');await page.waitForTimeout(80);positions.push(await searchPosition());
    ...
    const found=page.locator('.quick-child').filter({hasText:'Amira'});await found.waitFor();
    ...
    assert.equal(await page.locator('.page-heading').isVisible(),false,`${name}: secondary content remains visible while searching`);
    await search.fill('');await page.waitForTimeout(80);positions.push(await searchPosition());await search.fill('Amira');await page.waitForTimeout(80);positions.push(await searchPosition());await search.fill('');await page.waitForTimeout(80);positions.push(await searchPosition());
    ...
    await page.locator('[data-nav="today"]').click();await ready();await page.getByLabel('Kind suchen').fill(surname);await page.locator('.quick-child:disabled').filter({hasText:surname}).waitFor();await page.getByLabel('Kind suchen').fill('');await child.locator('.presence-badge').waitFor();
    ...
    assert.match(await child.innerText(),/Tessa-Lina/);await child.locator('[data-undo]').click();await child.waitFor({state:'hidden'});await page.getByLabel('Kind suchen').fill(surname);const quickRecheck=page.locator('.quick-child').filter({hasText:surname});await quickRecheck.click();await page.locator('.quick-child:disabled').filter({hasText:surname}).waitFor();await page.getByLabel('Kind suchen').fill('');await child.locator('.presence-badge').waitFor();
