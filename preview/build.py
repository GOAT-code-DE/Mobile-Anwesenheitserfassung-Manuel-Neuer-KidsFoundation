...
templates = templates.replace('Diesen vertraulichen Link persÃ¶nlich an die betreffende Person weitergeben. Er ist 24 Stunden gÃ¼ltig und wird nur hier angezeigt.', 'Demo: Es wurde kein echter Zugang angelegt und keine Einladung versendet. Der Link fÃ¼hrt zurÃ¼ck zur Vorschau.')
layout = (app / 'Pages/Shared/_Layout.cshtml').read_text()
shell = layout[layout.index('    <aside'):layout.index('\n}\nelse')]
shell = re.sub(r'@if \(demo\) \{.*?\n', '<div class="demo-banner"><span>Ã–FFENTLICHE DEMO</span> Nur erfundene Daten Â· keine echten Kinderdaten eingeben. Ã„nderungen bleiben nur bis zum Neuladen.</div>\n', shell)
...
shell = shell.replace('<footer class="app-footer">', '<footer class="app-footer"><button class="reset-link" id="demo-reset">Demo zurÃ¼cksetzen</button>')

login = '''<main id="welcome"><div class="auth-card"><a class="brand" href="?page=account"><img src="assets/brand/foundation-logo.svg" alt="Manuel Neuer Kids Foundation" width="632" height="150"></a><div class="eyebrow">UNSER ALLTAG. GUT ORGANISIERT.</div><h1>SchÃ¶n, dass du da bist.</h1><p>Entdecke die Anwesenheits-App fÃ¼r Gelsenkirchen und Bottrop.</p><div class="demo-entry"><span class="tag">Ã–FFENTLICHE TESTVERSION</span><h2>Die App ausprobieren</h2><p>Alle Kinder und Anwesenheiten sind erfunden. Bitte keine echten Kinderdaten eingeben. Ã„nderungen bleiben nur bis zum Neuladen.</p><div class="stack"><button class="button primary" data-role="manager">Testen starten</button></div></div><p class="small muted" style="margin-top:24px;margin-bottom:0">Ohne Anmeldung ausprobieren und den Link weitergeben. Starte als Hausleitung mit beiden Standorten. Andere Rollen kannst du oben in der App auswÃ¤hlen.</p><div id="boot-error" class="notice error" hidden></div></div></main>'''
html = f'''<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex,nofollow"><meta name="referrer" content="no-referrer"><meta name="csrf-token" content="public-synthetic-demo"><meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; font-src 'self' data:; img-src 'self' data:; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'none'"><title>Demo Â· NEUER KIDS</title><link rel="icon" href="assets/brand/foundation-logo.svg"><link rel="stylesheet" href="assets/css/brand-fonts.css?v={asset_version}"><link rel="stylesheet" href="assets/css/app.css?v={asset_version}"><link rel="stylesheet" href="assets/css/foundation.css?v={asset_version}"><style>.account #user-name,.account .user-avatar{{display:none}}#demo-role{{max-width:160px}}@media(max-width:760px){{.topbar{{padding:0 12px;gap:4px}}.topbar select{{font-size:.75rem;padding-right:4px}}#demo-role{{max-width:115px}}.account{{gap:2px}}.location-control{{gap:4px}}.location-icon{{display:none}}}}</style><script src="export.js?v={asset_version}" defer></script><script src="demo.js?v={asset_version}" defer></script></head><body class="auth-shell" data-page="account"><a href="#main" class="skip-link">Zum Inhalt</a><div id="demo-root">{login}</div><template id="login-template">{login}</template><template id="shell-template">{shell}</template>{templates}<div id="toast" class="toast" role="status" aria-live="polite" hidden></div><noscript><div class="notice">Bitte JavaScript aktivieren, um die Demo auszuprobieren.</div></noscript></body></html>'''
...
js = js.replace('  init();', "  window.neuerKidsDemo.dispose = () => {clearTimeout(toastTimer);++queryVersion;};\n  init().finally(() => document.body.dataset.ready = 'true');")
