...
(() => {
  ...
  function renderChildren() {
    ...
    $('#children-list').innerHTML = list.length ? list.map(c => `<article class="child-row"><div class="child-avatar" aria-hidden="true">${esc(c.firstName[0] + c.lastName[0])}</div><div class="child-main"><button type="button" data-edit="${c.id}">${esc(c.firstName)} ${esc(c.lastName)}</button><div class="child-meta"><span>${c.age} Jahre</span><span>·</span><span>${date(c.birthDate)}</span>${c.inactive ? '<span class="archive-badge">Inaktiv</span>' : ''}</div>${c.contactPhone ? `<a class="contact-link" href="tel:${esc(c.contactPhone.replace(/[^+0-9]/g,''))}">☎ ${esc(c.contactName)} · ${esc(c.contactPhone)}</a>` : ''}</div><div class="child-actions">${c.present ? `<span class="presence-badge">✓ Heute erfasst</span>${c.canUndo ? `<button type="button" class="icon-button" data-undo="${c.attendanceId}" aria-label="Anwesenheit von ${esc(c.firstName)} zurücknehmen" title="Eintrag zurücknehmen">↶</button>` : ''}` : `<button type="button" class="button small-button primary" data-attend="${c.id}">＋ Heute erfassen</button>`}<button type="button" class="icon-button" data-edit="${c.id}" aria-label="Profil von ${esc(c.firstName)} bearbeiten">⋯</button></div></article>`).join('') : `<div class="empty-state">${search ? 'Kein passendes Kind gefunden. Prüfe die Schreibweise oder beziehe inaktive Kinder ein.' : page === 'today' ? 'Noch kein Kind für heute erfasst. Suche oben nach einem Namen oder lege ein neues Kind an.' : 'Hier sind noch keine Kinder angelegt.'}</div>`;
  }
  ...
  async function init() {
    ...
    try{
      session=await api('/session');const preferred=Number(new URLSearchParams(location.search).get('site'));siteId=session.sites.find(s=>s.siteId===preferred)?.siteId||session.sites[0]?.siteId;
      $('#user-name').textContent=session.displayName;$('#admin-nav').hidden=!session.isAdmin;
      $('#site-picker').innerHTML=session.sites.length?session.sites.map(s=>`<option value="${s.siteId}">${esc(s.name)}</option>`).join(''):'<option>Administration</option>';$('#site-picker').value=siteId||'';$('#site-picker').disabled=session.sites.length<2;
